---
title: "HiTS-Kinetics-Visualization"
...
If we want to compare  HiTS-EQ and HiTS-Kin datasets, we need to visualize d = 4 data. Therefore the x axis is the position and nucleotide species and y axis is the paired nucleotide. The color indicates HiTS-EQ PWM+IC model parameters and the size shows the parameters of HiTS-Kin.

#Package request
ggplot2

#File list
Source code in R: Usage_of _Heatmap_bubble.R
HiTS-Kin dataset: HTSKIN_PWMIC.csv
HiTS-EQ dataset: HTSEQ_PWMIC.csv
#
...
For Data visualization, we need to normalize the data by the maximum value of the parameter. Here is the step for data nomralization to define the effect of each nucleotide/position.
```{r }
